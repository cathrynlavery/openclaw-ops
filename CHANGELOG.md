...
- The remediation workflow now explicitly requires a local board item for any real OpenClaw error/regression, hack/workaround, security concern, or recurring ops finding, preventing local operational issues from silently escaping the repair loop. Upstream links are optional metadata, not the primary workflow.
